...
const buildClientManifestFile = ({ entries, outputPath }) => {
    ...
    lines.push(`import type { CriticalImageResolver } from '../core/types';`);
    ...
    entries.forEach((entry, index) => {
        if (entry.criticalImageResolverImport) {
            lines.push(`const loadCriticalImageResolver${index} = async (): Promise<CriticalImageResolver> => {`);
            lines.push(`    const module = await import('${entry.criticalImageResolverImport}');`);
            lines.push(`    return module.default as CriticalImageResolver;`);
            lines.push('};');
            lines.push('');
        }
    });
