...
const namespacesOutput = path.resolve(__dirname, '../src/lib/i18n/namespaces.generated.ts');

const CORE_NAMESPACES = ['common', 'lobby', 'game', 'tutorial', 'auth'];
...
const collectGameEntries = async () => {
    ...
    for (const dirent of dirents) {
        ...
        const i18nPath = path.join(dirPath, 'i18n.json');
        ...
        const hasI18n = await fileExists(i18nPath);
        ...
        entries.push({
            ...
            i18nPath: hasI18n ? i18nPath : null,
        });
    }
...
const buildDataManifestFile = ({ entries, outputPath }) => {
    ...
    lines.push(`// AUTO-GENERATED by scripts/generate_game_manifests.js. DO NOT EDIT.`);
...
const buildClientManifestFile = ({ entries, outputPath }) => {
    ...
    lines.push(`import { DefaultGameThumbnail } from '../components/lobby/thumbnails';`);
    lines.push('');
    ...
    entries.forEach((entry, index) => {
        ...
        if (entry.thumbnailImport) {
            ...
            lines.push(`    thumbnail: <DefaultGameThumbnail titleKey={manifest${index}.titleKey} icon={manifest${index}.icon} />,`);
        }
        lines.push('};');
        lines.push('');
    });
...
const generateI18nFiles = async (entries) => {
    const gameNamespaces = [];

    for (const entry of entries) {
        if (!entry.i18nPath) continue;
        const raw = await fs.readFile(entry.i18nPath, 'utf8');
        const content = JSON.parse(raw);
        if (!content || typeof content !== 'object') {
            throw new Error(`[i18n] i18n.json 格式不合法: ${entry.i18nPath}`);
        }
        const namespace = `game-${entry.id}`;
        gameNamespaces.push(namespace);

        for (const [lang, payload] of Object.entries(content)) {
    ...
    const namespaces = [...CORE_NAMESPACES, ...gameNamespaces.sort()];
    const lines = [];
    lines.push('/* eslint-disable */');
    lines.push(`// AUTO-GENERATED by scripts/generate_game_manifests.js. DO NOT EDIT.`);
    lines.push(`export const I18N_NAMESPACES = ${JSON.stringify(namespaces, null, 2)} as const;`);
    lines.push('');
    await fs.writeFile(namespacesOutput, lines.join('\n'), 'utf8');
};
...
const run = async () => {
    ...
    await generateI18nFiles(entries);
    ...
    console.log(`- ${path.relative(process.cwd(), namespacesOutput)}`);
};
