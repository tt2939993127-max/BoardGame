...
async function openScene(game: any): Promise<void> {
    ...
    await game.setupScene({
        gameId: 'smashup',
        player0: { hand: [], deck: [], discard: [], vp: 0, factions: ['pirates', 'robots'] },
        player1: { hand: [], deck: [], discard: [], vp: 0, factions: ['ninjas', 'dinosaurs'] },
        currentPlayer: '0',
        phase: 'playCards',
        bases: [
            {
...
test.describe('多基地计分完整流程', () => {
    test('3个基地依次计分，中间有 afterScoring 交互', async ({ game }, testInfo) => {
        await openScene(game);
        await game.screenshot('multi-score-initial', testInfo);

        await game.advancePhase();
        await game.waitForInteraction('multi_base_scoring', 15000);
        await game.screenshot('multi-score-first-choice', testInfo);

        await selectBaseByDefId(game, 'base_the_jungle');
        await game.waitForInteraction('multi_base_scoring', 15000);
        await game.screenshot('multi-score-second-choice', testInfo);

        await selectBaseByDefId(game, 'base_pirate_cove');
        await game.waitForInteraction('base_pirate_cove', 15000);
        await game.screenshot('multi-score-pirate-cove', testInfo);
        await game.selectOption('skip');

        await game.waitForInteraction('multi_base_scoring', 15000);
        await game.screenshot('multi-score-last-base-choice', testInfo);
        await selectBaseByDefId(game, 'base_ninja_dojo');

        await game.waitForInteraction('base_ninja_dojo', 15000);
        await game.screenshot('multi-score-ninja-dojo', testInfo);
        await game.selectOption('skip');
        ...
        expect(finalState.core.players['1'].vp).toBe(6);
    });
});
