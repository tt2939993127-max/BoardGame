...
test.describe('角色选择系统', () => {
    ...
    test('应该为实施中的角色显示标签', async ({ page }) => {
        const evidenceDir = join(process.cwd(), 'test-results', 'evidence-screenshots', 'character-selection.e2e', '应该为实施中的角色显示标签');
        ...
        await expect(gunslingerBadge).toContainText('实施中');
        await expect(samuraiBadge).toBeVisible();
        await expect(samuraiBadge).toContainText('实施中');
        ...
        expect(gunslingerBadgeBox, '枪手实施中标签应存在边界框').not.toBeNull();
        ...
        expect(Math.abs(badgeCenterY - cardCenterY), '实施中标签应位于角色卡中部而不是角落').toBeLessThan(gunslingerCardBox!.height * 0.18);
        expect(gunslingerBadgeBox!.width, '实施中标签宽度应明显大于角标样式').toBeGreaterThan(gunslingerCardBox!.width * 0.5);
