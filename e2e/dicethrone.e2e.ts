...
import { test as harnessTest, expect as harnessExpect } from './framework';
...
harnessTest.describe('DiceThrone Harness E2E', () => {
    harnessTest('main flow: moon elf reaches defensive roll', async ({ page, game }, testInfo) => {
        harnessTest.setTimeout(DICETHRONE_TEST_TIMEOUT_MS);
        ...
        await harnessExpect(ui.handArea).toBeVisible({ timeout: 10000 });
        ...
        harnessExpect(state.sys.phase).toBe('defensiveRoll');
        harnessExpect(state.core.pendingAttack?.attackerId).toBe('0');
        harnessExpect(state.core.pendingAttack?.defenderId).toBe('1');
        harnessExpect(state.core.pendingAttack?.sourceAbilityId).toBe('longbow-5-1');
        harnessExpect(state.core.pendingAttack?.damage).toBe(7);

        await game.screenshot('01-main-flow-defensive-roll', testInfo);
    });

    harnessTest('regression: targeted adds 2 damage and persists', async ({ page, game }, testInfo) => {
        harnessTest.setTimeout(DICETHRONE_TEST_TIMEOUT_MS);
        ...
        harnessExpect(beforeState.core.players['1'].resources[RESOURCE_IDS.HP]).toBe(50);
        harnessExpect(beforeState.core.players['1'].statusEffects[STATUS_IDS.TARGETED]).toBe(1);
        ...
        harnessExpect(afterState.sys.phase).toBe('main2');
        harnessExpect(afterState.core.players['1'].resources[RESOURCE_IDS.HP]).toBe(45);
        harnessExpect(afterState.core.players['1'].statusEffects[STATUS_IDS.TARGETED]).toBe(1);

        await game.screenshot('02-targeted-damage-regression', testInfo);
    });

    harnessTest('ui stability: die lock toggle syncs state', async ({ page, game }, testInfo) => {
        harnessTest.setTimeout(DICETHRONE_TEST_TIMEOUT_MS);
        ...
        await harnessExpect(firstDieButton).toHaveAttribute('data-clickable', 'true', { timeout: 10000 });
        ...
        harnessExpect(lockedState.core.dice[0].isKept).toBe(true);
        ...
        harnessExpect(unlockedState.core.dice[0].isKept).toBe(false);
    });
});
