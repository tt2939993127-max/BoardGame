...
const fetchLobbyMatch = async (matchID: string): Promise<LobbyMatch | null> => {
    try {
        ...
        // 从游戏状态 G.__setupData 中读取房间名
        const setupData = match.state?.G?.__setupData as { roomName?: string } | undefined;
        const roomName = setupData?.roomName;
        return buildLobbyMatch(matchID, match.metadata, roomName);
    } catch (error) {
...
const fetchMatchesByGame = async (gameName: SupportedGame): Promise<LobbyMatch[]> => {
    try {
        ...
        for (const matchID of matchIDs) {
            ...
            const setupData = match.state?.G?.__setupData as { roomName?: string } | undefined;
            const roomName = setupData?.roomName;
            results.push(buildLobbyMatch(matchID, match.metadata, roomName));
        }
        return results;
    } catch (error) {
