...
- 如果用户反馈的是**游戏内**问题，主验收截图必须来自对应的游戏内现场；禁止拿大厅页、独立预览页、诊断页或脱离流程的组件截图替代“问题本身已修复”的证据
- 截图必须能直接证明“原问题位点现在是什么样子”，而不是只证明“相关资源存在”“某张图在别处能显示”或“某个组件单独渲染正常”
- 如果用户点名的是**具体控件/具体展开项**（如 `undo` 展开、某个 HUD 面板、某个弹窗步骤），主截图必须拍到**该控件本身处于问题对应状态**；禁止改测同一个共享组件里的别的按钮、别的展开项、或另一个更容易出图的兄弟场景来充当证据
- 即使问题根因位于共享组件层，验收截图也必须优先展示“用户报错的那个入口/那个按钮/那个展开态”已经恢复；只有在用户明确接受的前提下，才允许用其他游戏或其他入口的共享组件截图作辅证
- 凡是展开/弹层/下拉/抽屉/悬浮面板类问题，主截图必须同时拍到：触发按钮本体、展开内容的完整边界、以及能判断相对位置的兄弟元素或屏幕边界；如果展开部分被裁掉、只拍到一角、或看不出它相对谁展开，默认视为“未证明修复”
- 如果主截图已经包含完整上下文，但展开边界、锚点位置、与兄弟按钮/屏幕边界的贴合关系仍然看不清，必须再补一张局部放大图；完整图负责证明上下文，局部图负责证明边界细节，两者缺一不可
- “按钮亮了”“有半透明块出现了”“测试断言说 panel 可见”都不算展开类问题的充分证据；必须从截图里肉眼判断展开面板的完整位置、尺寸和遮挡关系
...
4. 如果问题发生在游戏内流程里，主截图必须来自该流程中的真实问题现场；大厅图、诊断图、资源图、组件单独预览图只能作辅证，不能代替主验收图。
5. 如果问题描述里已经点明具体控件（例如“撤回展开位置不对”“设置面板挡住日志”“某个按钮的二级菜单错位”），主截图必须把该控件展开后的实际位置完整拍出来，并让人一眼看出它相对于兄弟按钮/被遮挡区域的关系；只拍同套 HUD 里的其他按钮，不算命中问题现场。
5.1. 如果主截图已经能看出完整上下文，但控件边缘、展开边界或锚点细节仍不够清晰，必须补一张针对问题位点的局部放大图；没有局部放大图，就不能宣称“边界细节已确认无误”。
5. 如果证据文档中嵌入了多张图，必须保证每张图旁边都有自己的绝对路径，不能只在文档开头给一个总目录。
6. 线上问题必须至少附一张线上环境现状截图；本地修复截图不能替代线上现状截图。
7. 黑图、纯加载页、纯空白页、只有遮罩或只有单个噪点而看不出业务界面的截图，一律视为“无有效截图”，不得拿来充当 UI 验收证据。
8. 只要首张截图已经表现为“无有效截图”，就必须立刻在结论里写明“本轮没有拿到有效业务截图”，不能继续用这轮产物宣称“已看图确认正常”。
9. 未亲自打开过的截图路径不得汇报给用户作为验收依据；如果只是文件存在、尚未看图，只能表述为“产物路径”，不能表述为“已核对截图”。
10. UI / 移动端适配 / 布局 / 动画 / 触屏交互类 E2E，必须额外逐项核对这些视觉项：
