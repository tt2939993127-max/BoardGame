...
| 守卫、燃烧、腐化、眩晕、昏迷、沉睡 token | 状态规则与逐卡字段 | 是，状态出现时可见 | 对应状态 token 图 | `foundation-runtime-ready-for-guard-and-damage / full-status-tooltip-deferred-after-foundation` | 当前 Board 已接守卫和伤害相关 token；其它基础状态素材已进资源链 | 通用彩色 badge、emoji、纯文字替代 | 完整状态层数和 tooltip 留到 foundation 后 |
