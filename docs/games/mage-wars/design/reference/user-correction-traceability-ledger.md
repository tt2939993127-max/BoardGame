...
| 法师提示卡和代表法师本人的卡不是一个东西 | 把 HUD 提示卡、竞技场法师实体和完整规则卡混用，会导致提示挂错层、战场实体职责被 UI 壳替代，且 E2E 用含混命名把错误固化 | `mage-wars-ui-design-memory` 法师三对象强制拆分；TTS / atlas 证据：`mages-core-atlas.json` 中 `2600/2605/2606/2603` 为完整规则卡，`2601/2604/2607/2602` 为肖像 | 规则对象覆盖矩阵必须拆 `法师战场实体`、`玩家 / 法师提示卡`、`法师规则卡` 三行；代码 / 测试必须使用 `mage-entity`、`mage-hud-hint-card`、`mage-rules-card` 等职责命名，不得再用 `identity-card` 混称 | covered |
...
- [x] 法师战场实体、玩家 / 法师提示卡、法师规则卡已在矩阵、实现和测试中分开命名、分开素材职责、分开验收；不再使用 `identity-card` 混称。
