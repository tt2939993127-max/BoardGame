...
17. **用户点名的比例异常必须验“相对比例”**：如果用户明确指出某个区域“太小 / 太大 / 权重不对”，除了断言它在屏内、未裁剪，还必须补一条相对主结构的比例检查，例如“底部 rail 相对主卡阵的宽度比例不能明显低于桌面版”；禁止只验“看得见”就宣布收口
18. **移动端对照规范必须同时检查 PC 构图是否被带坏**：凡是“桌面同构缩放”类适配，验收时必须同时打开移动端图和 PC 对照图，逐项检查主卡阵是否仍处于视觉中心、列内卡片是否居中、标题与主卡阵的中轴是否一致；禁止只盯移动端截图，放任 PC 图出现整体偏左、主构图失去中轴的回归
19. **底部玩家区必须按“可辨识结构”验收，不得只验占位存在**：像玩家卡片、派系槽位、当前玩家状态这类底部信息条，验收时必须确认头像、槽位、状态文案/标识仍可肉眼分辨；如果截图里只剩一条色块、几个无法辨认的小图标或“看得见但读不出”，一律视为未通过
