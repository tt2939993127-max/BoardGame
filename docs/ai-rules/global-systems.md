...
- **安卓构建物也必须网页安全**：
    - 只要同一份前端静态资源存在被浏览器直接访问的可能，就必须假设“网页可能拿到 android build”。
    - 因此即使是 `MODE === 'android'` 的构建产物，用户可见 UI、自动副作用、原生插件调用、语言能力收缩、网络握手策略等仍必须以“网页安全默认值”为基线，再由真实原生运行时覆盖。
- **必须使用统一运行时探测**：
    ...
    - `window.androidBridge`、自定义 JS bridge、第三方容器注入对象这类信号，**不得单独作为“当前就是原生 App” 的充分条件**；只有在已验证的原生平台探测同时成立，或已有明确文档证明该 bridge 只会在目标壳层内注入时，才能作为补充诊断信息。
- **入口层与组件层双重门禁**：
...
- **共享组件默认站在网页安全侧**：
    ...
    - 共享组件禁止内联读取 `window.Capacitor` / `window.androidBridge` 自行做分支；必须复用统一 helper，保证 OTA、原生更新、包管理、返回桥等所有入口的判定口径一致。
- **单一真实来源（强制）**：
    - 共享组件、Hook、service、plugin wrapper 只允许复用统一 helper，禁止在组件内直接手写 `window.Capacitor`、`Capacitor.isNativePlatform()`、`Capacitor.getPlatform()` 或自定义同义判断。
    - 任何新边界判断若确实需要补充探针，必须先回收敛到统一 helper，再由调用方复用；禁止边写边分叉出第二套标准。

### 禁止模式（强制）

- **禁止**用 `MODE === 'android'` 直接控制以下行为：
    - OTA / 原生更新 UI 显示
    - 返回桥监听
    - 原生插件调用
    - 游戏包安装入口显示
    - 语言能力裁剪
    - Socket 握手策略切换
- **禁止**在共享 UI 组件里直接读取 `window.Capacitor`
- **禁止**在多个模块里各自实现“是否原生安卓”的判断
- **禁止**因为“当前是 app 需求”就把壳层专属逻辑直接散落到 Home / HUD / 弹窗等网页共用组件
...
- `MODE === 'android'` 只允许用于：
    ...
    - 剪除只在壳层构建里需要的包体内容
- 真实运行时判断只允许用于：
    - 是否挂载 OTA / Native Update Manager
    - 是否显示“检查更新”“安装游戏包”等壳层入口
    - 是否调用 Capacitor 原生插件
    - 是否监听原生返回、前后台、深链等壳层事件

### 边界审计清单（提交前至少自检一次）

- 搜索 `MODE === 'android'`
    - 确认命中点只在构建裁剪/静态配置层，不在用户可见行为层
- 搜索 `window.Capacitor`
    - 确认没有共享组件绕过统一 helper
- 搜索 `registerPlugin(`
    - 确认插件包装器内部仍有真实运行时门禁
- 搜索 `isNativePlatform(` / `getPlatform(`
    - 确认不是在重复实现边界判断，而是在统一 helper 内部
- 搜索 `androidBridge`
    - 确认桥接对象不会被网页端调试脚本、占位对象或 mock 代码泄漏到真实页面
