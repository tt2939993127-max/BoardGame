...
- 双边范围：`docs/ai-rules/ui-change-gates.md` 现行章节与文件末尾旧版 `0.1-0.4` blockquote 外壳。
...
- 主源回归：E2E 截图来源、状态触发、截图命名和证据文档要求回 `docs/ai-rules/e2e-verification.md`；PC/移动参考分辨率和双端布局回 `docs/ai-rules/ui-responsive-layout.md`；`ui-change-gates.md` 只保留改动门禁、空间与交互槽位裁决。
...
- 已将 `.codex/skill/game-audit-workflow/SKILL.md` 的「默认执行口径」长红线原样拆到 `.codex/skill/game-audit-workflow/references/audit-redlines.md`。
...
- 已将 `docs/ai-rules/testing-audit.md` 中「审计文档最低模板」和「审计后自检」长正文压缩为模板入口。
- `docs/ai-rules/audit-evidence-template.md` 作为 evidence 模板主源，承接审计范围、结论等级、权威来源、逐项结论、验证证据、共享根因、修订记录、继续任务防重复门禁、测试语义对账与同类扩审。
...
- 已将 `docs/ai-rules/testing-audit.md` 中「深度审计流程」Step 0-5 与深审禁区无损迁入 `docs/ai-rules/testing-audit-core-principles.md`。
...
- 已复核 `docs/ai-rules/testing-audit.md` 中「禁止假阳性收口」与 `testing-audit-core-principles.md`、`game-audit-workflow` 红线的关系。
...
- 已将 `docs/ai-rules/testing-audit.md` 的「回归问题处理流程」长正文压缩为回归处理入口。
- `docs/ai-rules/regression-closeout.md` 作为回归收口主源，承接原 testing-audit 中独有的回归细节：不得改写用户症状、数量级不得降级、未复现要直说、最后正常证据、`git blame` / `git log -S/-G` / `git show` / `git bisect`、引入 hunk diff、默认还原错误 hunk、首跑红测保留、不得换场景绕过失败、原始位点 E2E、UI 最小还原例外、代理按钮合同和输出模板。
...
- 已将 `docs/ai-rules/testing-audit.md` 的「Bug 修复后的同类扩审」细则压缩为同类扩审入口。
- `docs/ai-rules/regression-closeout.md` 的「同类扩审最低要求」承接搜索维度、共享层覆盖、命中处理和交付口径。
...
- 已复核 `docs/ai-rules/testing-audit.md` 中「指定最近合并 PRxx 为权威基线时的红测归因」与 `AGENTS.md`、`.codex/skill/merge-pr-workflow/`、`docs/git-merge-checklist.md`、`testing-audit-core-principles.md` 的关系。
...
- 已将 `docs/ai-rules/testing-audit.md` 中「根因分级与处置」长正文迁入 `docs/ai-rules/testing-audit-core-principles.md`。
...
- 已复核 `docs/ai-rules/testing-audit.md` 中「测试工具选型」「效果数据契约测试」「交互链完整性审计」「CI 质量门禁」与 `docs/automated-testing.md`、`docs/testing-best-practices.md`、`testing-audit-core-principles.md` 的关系。
...
- 已将 `docs/ai-rules/testing-audit.md` 中「E2E 测试选择器一致性检查」的检查清单和反模式迁入 `docs/automated-testing.md`。
...
- 已复核 `docs/automated-testing.md` 中「截图人工核对」「E2E 截图核对补充规范」「外部资源缺失时的看图规则」与 `docs/ai-rules/e2e-verification.md` 的关系。
...
- 双边范围：`docs/ai-rules/testing-audit.md` 的“审计结论等级与证据分层”长段，与 `docs/ai-rules/audit-evidence-template.md`、`docs/ai-rules/testing-audit-core-principles.md` 的结论、证据和范围门禁；另检查其末尾“教训附录入口 / D 维度库入口”重复。
...
- 双边范围：`.codex/skill/game-audit-workflow/references/dimensions.md` 的 D1-D52 简表，与 `docs/ai-rules/testing-audit-dimensions.md` 的完整 D1-D58 维度库。
...
- 已将 `.codex/skill/data-entry-workflow/SKILL.md` 从长门禁正文压缩为 BoardGame 录入 workflow 骨架。
- `docs/ai-rules/data-entry.md` 继续作为通用录入原则、来源优先级、核对契约和零猜测门禁的 `canonical-source`。
- `docs/ai-rules/asset-pipeline.md` 承接图片/素材路径、manifest、正式资源链和服务器素材主源；`.codex/skill/safe-image-reading/SKILL.md` 承接读图 / OCR 执行纪律；`docs/ai-rules/engine-systems.md` 承接机制实现前置审查。
...
- 已复核 `.codex/skill/atlas-crop/SKILL.md`：当前约 82 行，结构为职责边界、适用场景、脚本、快速流程、验收要点和少量项目内图集类型摘要。
- 该 skill 已明确降为 `workflow`，正式素材主源 / manifest / 运行时资源链回 `docs/ai-rules/asset-pipeline.md`，录入核对合同回 `docs/ai-rules/data-entry.md`。
...
- 双边范围：`docs/ai-rules/audio-assets.md` 与 `docs/audio/audio-usage.md` 都曾承载三层架构、registry key 禁令和已安装包 / 共享音频路径合同；两者还分别承载运行时触发规则与项目级命令 / 查找 / BGM 接入细节。
- 主源裁决：跨游戏运行时架构、FeedbackPack / 音效触发时机和共享包四层路径合同归 `docs/ai-rules/audio-assets.md`；资源目录、生成命令、候选查找、试听、BGM 和游戏接入归 `docs/audio/audio-usage.md`。
- 本轮动作：删除 `audio-usage.md` 中重复的架构与本地包路径正文，改成主源入口；删除 `audio-assets.md` 中重复的命令列表，改成使用合同 / workflow 路由。
- 额外修正：`audio-usage.md` 原先指向不存在的项目 `.codex/skill/audio-integration/`，已统一改为系统 skill `D:\codex-home\skills\audio-integration\SKILL.md`。
...
- 范围：`docs/ai-rules/asset-pipeline.md`、`docs/ai-rules/critical-image-preload.md`、`.codex/skill/atlas-crop/SKILL.md`。
...
- 已把 `.codex/skill/create-new-game/SKILL.md` 顶部 19 条块引用红线无损搬到 `references/intake-redlines.md`。
...
- 已把 `.codex/skill/create-new-game/SKILL.md` 阶段 0 的最小闭环、最低产物和一票否决迁入 `references/intake-redlines.md`。
...
- 日常 Git 操作、提交/推送、提交粒度和快速路径细节统一回 `.codex/skill/git-operations/SKILL.md`。
- 分支/worktree 锁定标准回 `docs/ai-rules/worktree-branch-target-lock.md`；PR 合并执行回 `.codex/skill/merge-pr-workflow/SKILL.md`；用户合并裁决包回 `.codex/skill/merge-decision-package/SKILL.md`。
- 这是根规范压缩，不改变禁止无授权改 Git 状态、禁止擅自删边、高风险合并需语义验证等硬红线。

### P1/P3 根段落级合并（2026-08-08）

- 已压缩 `AGENTS.md` 资源段：根文件只保留图片/音频资源入口和硬红线，完整压缩、locale、图集、manifest、服务器素材主源、移动包与音频合同回 `docs/ai-rules/asset-pipeline.md`、`docs/ai-rules/critical-image-preload.md`、`docs/ai-rules/audio-assets.md`。
...
- 已把不存在的项目内 `.codex/skill/audio-integration/SKILL.md` 引用修正为系统 skill `D:\codex-home\skills\audio-integration\SKILL.md`；音频规范正文仍由 `audio-assets.md` 承载。
...
- “检查/审查/看一下然后提交”、静态分析、hook 依赖、快速路径和 warning-only 口径回前文“执行优先级与归并口径”及 `.codex/skill/git-operations/SKILL.md`，根文件不再重复审查模板。
...
- 引用验证：`docs/README.md`、`docs/ai-rules/doc-index.md`、根 `AGENTS.md` 和现有 evidence 均指向 `docs/games/dicethrone/workflows/dicethrone-hero-intake.md`；仓内没有有效引用旧顶层路径。
...
- 引用验证：根 `AGENTS.md`、`docs/ai-rules/data-entry.md`、现有 evidence 和 OpenSpec 资料均指向 `docs/games/smashup/workflows/smashup-faction-intake.md`；未发现有效业务入口引用旧顶层路径。
...
- 本轮动作：整体迁入 `docs/games/smashup/refactor/pod/`，保留正文不删语义，并更新 `docs/ai-rules/doc-index.md`、`src/games/smashup/rule/POD-SYSTEM.md` 与 POD 文档内部引用。
...
- 验证口径：`docs/ai-rules/doc-index.md`、`POD-SYSTEM.md`、bug 文档和 POD 文档内部引用均回到现存入口；仓内不再有对已删除选择性覆盖文档的业务引用。
...
- 保留侧：音频内容归并到 `docs/ai-rules/audio-assets.md` 的“音效触发与迁移策略”；Pixi 内容归并到 `docs/ai-rules/animation-effects.md` 的“PixiJS 选型结论”。
...
- 双边范围：根 `AGENTS.md` 的生产部署 / Android OTA 细则，与 `.codex/skill/android-app-release/SKILL.md`、`docs/mobile-release.md`、`docs/android-app-build.md`、`docs/deploy.md` 的专项合同。
