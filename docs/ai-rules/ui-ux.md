...
- **PC 主参考分辨率固定为 `1920x1080`**。
  - 这是桌面端唯一默认权威基线；未特别说明时，桌面截图、桌面布局对照、PC 同构验收都优先以此为准。
  - 禁止在同一轮验收中无说明地混用 `1440x900`、`1600x900`、`1366x768` 作为“主参考图”。这些分辨率只能作为补充覆盖，不是主裁决基线。
- **PC 补充分辨率固定为 `1366x768`**。
  - 用于验证较小笔记本视口下是否仍可用、无遮挡、不过度挤压。
  - 它是补充门槛，不得反过来覆盖 `1920x1080` 的桌面主态结论。
- **平板主参考分辨率固定为 `1024x768`（横屏）**。
  - 平板默认按 PC 风格验收，关注结构一致与无遮挡，不按“手机壳比例”验收。
- **手机横屏主参考分辨率固定为 `800x450`（16:9）**。
  - 用户未另行指定时，移动端横屏截图、适配验收、对照证据默认使用它。
  - 禁止再用 `812x375` 这类超宽设备尺寸替代项目默认横屏基线，再把比例差异误判为产品问题。
- **手机竖屏主参考分辨率固定为 `375x812`**。
  - 仅用于 `portrait-adapted` 游戏或明确要求竖屏的流式信息页；不得拿它替代横屏游戏的主验收视口。

### 2.1 参考分辨率使用规则（强制）

- 先看该界面的**主参考分辨率**结果，再决定布局是否成立；补充分辨率只用于查边界，不用于推翻主基线。
