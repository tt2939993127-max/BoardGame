...
- **E2E 证据必须直击业务问题（强制）**：凡是用 E2E 证明“用户这次报的问题已修复”，必须覆盖该问题的真实业务链路、真实触发时机和真实问题位点。禁止只用一个自造代理场景、一个通用机制示例、或一张与问题弱相关的截图，就对外宣称原问题已通过。
- **看图必须核对业务关联局部（强制）**：截图验收时必须精确检查与问题直接相关的区域，例如具体候选列表、具体卡牌、具体基地位、具体弹层步骤、具体遮挡区；不能只看“页面能打开”“有界面”“最终大状态差不多对”。
- **证据链必须包含“问题位点已消失”的直接证明（强制）**：如果问题是“某列表仍包含旧对象”“某阶段仍显示已失效目标”“某步交互仍拿旧快照”，证据至少要能证明两件事：① 问题发生的那一步里旧对象已经不再出现 ② 后续结算结果也正确。只证明最终结果正确、不证明中间问题位点已消失，不足以收口。
