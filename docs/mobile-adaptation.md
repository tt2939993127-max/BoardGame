...
- 本项目默认 **PC 主参考分辨率** 为 `1920x1080`。
- 本项目默认 **PC 补充分辨率** 为 `1366x768`，用于验证较小笔记本视口下是否仍可用、无遮挡、不过度挤压；它只能作为补充覆盖，不能替代 `1920x1080` 的主裁决地位。
- 本项目默认 **平板主参考分辨率** 为 `1024x768`（横屏），按 PC 风格验收结构一致性与无遮挡，不按手机壳比例验收。
- 本项目默认 **手机横屏主参考分辨率** 为固定 `16:9`；用户未另行指定时，优先使用 `800x450`。
- 本项目默认 **手机竖屏主参考分辨率** 为 `375x812`，仅用于 `portrait-adapted` 游戏或明确要求竖屏的流式信息页。
- 若用户明确说明“平板按 PC 看”或“这轮不关心平板”，则该轮移动端验收可以不单独补平板横屏档。
- 若老板明确指定分辨率（例如 `1920x1080`），则以老板指定为准；未指定时按上述默认值执行。
