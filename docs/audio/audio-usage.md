...
- **行为语义优先于素材类别**：只要玩家感知到的核心动作是“埋藏 / 翻开隐藏牌 / 封印 / 解除封印 / 召回 / 传送 / 变形 / 召唤 / 献祭 / 吞噬 / 检视隐藏区”等机制语义，就必须优先选择表达该语义的 key，不能因为载体是卡牌，就默认落到 `card.handling.*` 或 `card.fx.*`。
- **抽象卡牌音只用于通用移牌**：`card.handling.*`、`card.fx.*` 仅用于“打出、抽取、弃置、洗切、移动到牌库顶/底、在公开区域之间转移”这类没有更强机制语义的通用卡牌操作。
- **复合语义事件取最强语义**：如果一个事件同时包含“特殊机制 + 卡牌位移”，优先表达前者。例如“翻开一张埋藏牌并回手”应先按“翻开/解除隐藏”选音，而不是按“回手/抽牌”选音。
- **明确禁止**：隐藏区放置、翻开隐藏牌、封印、解封、出土、伏击显现、秘密区检视、特殊召回等事件，禁止使用 `cards_scrolling_001`、`card_take_001` 这类纯卡牌 handling 音作为默认映射，除非经过单独审查并有文档说明为何没有更贴切的语义音。
...
- **通用音效池优先**：每个阵营/牌组在 `audio.config.ts` 中配置通用音效池（单位池 + 行动池），同类卡牌默认使用通用音效池随机播放。
