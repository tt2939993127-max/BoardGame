...
|---:|---|---|---|---|---|
| 18 | `upgrade-blink-2` | 瞬身 II | `blink` | 2026-06-06 已确认不是升级卡接错，而是升级技能的防御重投合同只接到了“还能再掷几轮”，漏掉了“每轮至多重掷 2 颗”；现已补 `trigger.rollLimit = 2 + rerollDieLimit = 2`，并用合同测试锁定第二次防御掷骰前不能放行 3 颗全重掷。2026-06-05 再复跑时另确认真实红点曾切到 `DiceTray` 旧骰面命中层，现已由 `DiceTray.tsx` 命中层补丁收口 | **升级卡接线正确；共享实现漏项与 UI 命中层回归均已修，关键 L4 已锁** |
...
| 29 | `upgrade-death-blossom-2` | 死亡盛放 II | `death-blossom` | 当前 `DEATH_BLOSSOM_2` 已切到升级版奖励骰合同：按忍刀/手里剑累计伤害，`1` 面具不可防御，`2` 面具施加慢性中毒，并支持至多 `2` 次奖励骰重掷；当前费用合同为不绑定 Token、费用 `0`，对外口径写“技能内置重投 / 不消耗 Token 或资源”，不把卡图规则文本改名为“免费重投”；关键面具数量分层 `L4` 已补齐 | **已对齐；对象级 L3 已补齐，关键 L4 已锁，剩批次级治理与旧文档统一收口** |
