...
const REQUIRED_SOURCE_CONFIGS: Record<string, { targetType?: string; autoRefresh?: string; responseValidationMode?: string }> = {
    ...
    base_inventors_salon: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    ...
    cthulhu_recruit_by_force: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    cthulhu_it_begins_again: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    cthulhu_madness_unleashed: { targetType: 'hand' },
    cthulhu_chosen_confirm: { targetType: 'minion' },
    cthulhu_star_spawn: { targetType: 'generic' },
    cthulhu_servitor: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    special_madness: { targetType: 'button' },
    elder_thing_begin_the_summoning: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    ...
    ancient_egyptians_pyramid_engineer_uncover: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    ancient_egyptians_lost_knowledge_uncover: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    ancient_egyptians_pharaoh_before_scoring: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    ancient_egyptians_seal_the_tomb_uncover: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    titan_sphinx_start_turn: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    titan_sphinx_after_scoring: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    bury_uncover_start_turn: { targetType: 'generic', autoRefresh: 'buried', responseValidationMode: 'live' },
    ...
    robot_microbot_reclaimer: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    robot_hoverbot: { targetType: 'generic', responseValidationMode: 'live' },
    steampunk_scrap_diving: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    ...
    wizard_mass_enchantment: { targetType: 'generic', responseValidationMode: 'live' },
    wizard_mass_enchantment_choose_base: { targetType: 'base' },
    wizard_mass_enchantment_choose_minion: { targetType: 'minion' },
    wizard_portal_order: { targetType: 'generic', responseValidationMode: 'live' },
    base_wizard_academy: { targetType: 'generic', responseValidationMode: 'live' },
    cowboys_gold_in_them_thar_hills: { targetType: 'generic', responseValidationMode: 'live' },
    cowboys_gold_in_them_thar_hills_order: { targetType: 'generic', responseValidationMode: 'live' },
    ...
    vikings_cast_the_runes_order: { targetType: 'generic', responseValidationMode: 'live' },
    vikings_raiding_party_choice: { targetType: 'generic', responseValidationMode: 'live' },
    ...
    zombie_grave_digger: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    zombie_walker: { targetType: 'button' },
    zombie_grave_robbing: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    zombie_not_enough_bullets: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    zombie_lend_a_hand: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    ...
    elder_thing_begin_the_summoning_pod: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    elder_thing_spreading_horror_pod_choose_minion: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    vampire_fledgling_vampire_pod_bury_source: { targetType: 'generic', autoRefresh: 'hand_or_discard', responseValidationMode: 'live' },
    vampire_crack_of_dusk: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    vampire_wolf_pact_pod_action: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
    titan_penguins_emperor_penguin_talent: { targetType: 'generic', autoRefresh: 'hand_or_discard', responseValidationMode: 'live' },
    titan_ghosts_creampuff_man_play: { targetType: 'generic', autoRefresh: 'discard', responseValidationMode: 'live' },
};
...
const APPROVED_GENERIC_SOURCE_REASONS: Record<string, string> = {
    ...
    base_wizard_academy: '候选会按当前 baseDeck 顶部仍连续保留的揭示块实时过滤与重建，保留 generic 以承载基地牌库顶上下文。',
    ...
    robot_hoverbot: '候选引用当前牌库顶；若顶部被改写，必须实时失效，不能继续保留旧揭示牌。',
    ...
    elder_thing_begin_the_summoning_pod: '从弃牌堆选择随从洗回牌库顶，候选直接引用 discard 卡面，需要 live 重验。',
    elder_thing_spreading_horror_pod_choose_minion: '从弃牌堆选择打出的随从，候选直接引用 discard 卡面，需要 live 重验。',
    trickster_block_the_path: '候选项是效果处理分支，不对应单一实体直选。',
    wizard_mass_enchantment: '候选会按各对手当前牌库顶仍可见的行动卡实时过滤与重建，保留 generic 以承载跨玩家牌库顶上下文。',
    wizard_portal_order: '候选会按当前仍位于牌库顶的揭示块实时过滤与重建，保留 generic 以承载揭示排序上下文。',
    ...
    titan_penguins_emperor_penguin_talent: '候选项混合手牌与弃牌堆的低战力随从，必须保留双来源卡面上下文。',
    base_drakkar: '候选项是抽象分支选择，不对应单一棋盘实体直选。',
    base_longhouse_card: '候选项来自展示牌库顶后的静态卡面，不对应当前手牌或棋盘实体。',
    cowboys_gold_in_them_thar_hills: '候选会按当前仍位于牌库顶的揭示块实时过滤与重建，保留 generic 以承载揭示卡面与分支上下文。',
    cowboys_gold_in_them_thar_hills_order: '候选会按当前仍位于牌库顶的揭示块实时过滤与重建，保留 generic 以承载揭示排序上下文。',
    cowboys_stagecoach_cards: '候选项混合场上目标与静态快照信息，不能压成单一实体直选。',
    innsmouth_return_to_the_sea_choose_name: '选择的是随从名称与对应场上候选快照，不是单一实体直选。',
    titan_cthulhu_cthulhu_titan_talent_target: '候选项混合场上随从与后续抽象效果上下文，保留 generic 更准确。',
    titan_ghosts_creampuff_man_play: '候选项混合手牌与弃牌堆来源，后续还要决定额外打出目标。',
    trickster_mark_of_sleep_pod: '交互需要同时保留目标玩家与其手牌上下文，不能压成单一 hand/player 直选。',
    vikings_berserk_card: '候选项来自展示牌库顶后的静态卡面，不对应当前棋盘实体。',
    vikings_cast_the_runes_order: '候选会按当前仍位于牌库顶的揭示块实时过滤与重建，保留 generic 以承载揭示卡面上下文。',
    vikings_cast_the_runes_player: '交互先选目标玩家再读取其牌库顶，保留 generic 以承载复合上下文。',
    vikings_huscarl: '候选项来自复合来源的战利品选择，不对应单一实体直选。',
    vikings_pillage: '交互候选包含复合来源的可掠夺卡牌，保留 generic 更准确。',
    vikings_raider: '候选项来自弃牌堆/埋葬牌等复合战利品来源，不对应单一实体直选。',
    vikings_raiding_party_choice: '候选会按当前仍位于牌库顶的揭示块实时过滤与重建，保留 generic 以承载揭示卡面与分支上下文。',
    vikings_raiding_party_player: '交互先选目标玩家再读取其牌库顶，保留 generic 以承载复合上下文。',
    vikings_ransack: '候选项来自复合来源的可掠夺卡牌，不能压成单一实体直选。',
    vikings_shield_maiden: '候选项来自复合来源的战利品选择，不对应单一实体直选。',
    vikings_valkyrie: '候选项来自复合来源的可回收战利品，不对应单一实体直选。',
};
