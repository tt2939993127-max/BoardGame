...
describe('响应窗口跳过逻辑', () => {
    ...
    it('同一基地已使用过影舞者后，下一轮应跳过手里剩余的影舞者', () => {
        const runner = new GameTestRunner<SmashUpCore, any, any>({
            domain: SmashUpDomain,
            systems: smashUpSystemsForTest,
            playerIds: ['0', '1'],
            ...
            setup: (playerIds: PlayerId[], random: RandomFn): MatchState<SmashUpCore> => {
                ...
                core.bases[0] = {
                    defId: 'base_the_mothership',
                    minions: Array.from({ length: 4 }, (_, i) => ({
                        uid: `scoring-${i}`,
                        defId: 'test_minion',
                        owner: '0' as const,
                        controller: '0' as const,
                ...
                core.scoringEligibleBaseIndices = [0];
                core.specialLimitUsed = { ninja_shinobi: [0] };

                core.players['0'].hand = [
                    { uid: 'blocked-shinobi', defId: 'ninja_shinobi', type: 'minion', owner: '0' },
                ];
                core.players['1'].hand = [];

                sys.responseWindow = {
                    current: {
                        id: 'me-first-repeat-shinobi',
                        responderQueue: ['0', '1'],
                        currentResponderIndex: 1,
                        passedPlayers: ['0'],
                        windowType: 'meFirst',
        ...
        const passResult = runner.dispatch('RESPONSE_PASS', { playerId: '1' });
