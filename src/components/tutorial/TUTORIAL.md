...
export interface TutorialStep {
    ...
    highlightTarget?: string; // 高亮目标 (通过 data-tutorial-id 匹配)
    position?: string;   // 建议位置（系统会自动智能计算最佳位置）
    requireAction?: boolean; // 是否需要用户执行特定操作才能继续
    aiMove?: number;     // AI 自动落子的索引。设置后，该步骤不显示气泡，AI 动作完成后自动进入下一步。
...
1. **AI 回合处理**：
   如果一个步骤包含 `aiMove`，`TutorialOverlay` 会**自动隐藏**。不要在 AI 回合步骤写长篇大论的内容，用户看不到。应该在 AI 回合之后添加一个新的纯文本步骤来解释刚才发生了什么。
...
3. **智能定位逻辑**：
   气泡定位优先级为：**右侧 > 左侧 > 下方 > 上方**。系统会自动检测边缘空间并进行 Clamping（防止超出屏幕）。
