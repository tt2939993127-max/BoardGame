...
const resolveTitle = (
    ...
    switch (state.phase) {
        case 'checking':
            return t('nativeUpdate.gate.checkingTitle');
        case 'downloading':
            return t('nativeUpdate.gate.downloadingTitle');
        case 'verifying':
            return t('nativeUpdate.gate.verifyingTitle');
        case 'permission-required':
            return t('nativeUpdate.gate.permissionTitle');
        case 'installing':
            return t('nativeUpdate.gate.installingTitle');
        case 'error':
            return t('nativeUpdate.gate.errorTitle');
        default:
            return '';
    }
};
...
const resolveDescription = (
    ...
    switch (state.phase) {
        case 'checking':
            return t('nativeUpdate.gate.checkingDescription');
        case 'downloading':
            return t('nativeUpdate.gate.downloadingDescription');
        case 'verifying':
            return t('nativeUpdate.gate.verifyingDescription');
        case 'permission-required':
            return t('nativeUpdate.gate.permissionDescription');
        case 'installing':
            return t('nativeUpdate.gate.installingDescription');
        case 'error':
            return t('nativeUpdate.gate.errorDescription');
        default:
            return '';
    }
};
                ...
                <section className="w-full max-w-[24rem] rounded-[18px] border border-amber-200/15 bg-[#161008]/92 p-6 text-center shadow-[0_30px_100px_rgba(0,0,0,0.55)] backdrop-blur-md">
                    ...
                    {state.version && (
                        <div className="mt-5 rounded-2xl border border-amber-200/12 bg-black/20 px-4 py-3 text-left text-xs text-amber-100/70">
                            <div className="flex items-center justify-between gap-3">
                                <span>{t('nativeUpdate.gate.targetVersion')}</span>
                    ...
                    {showProgress && (
                        <div className="mt-5">
                            <div className="mb-2 flex items-center justify-between gap-3 text-xs text-amber-100/70">
                                <span>{t('nativeUpdate.gate.progressLabel')}</span>
                                <span>
                                    {typeof progressPercent === 'number'
                                        ? t('nativeUpdate.gate.progressPercent', { percent: progressPercent })
                                        : t('nativeUpdate.gate.progressPending')}
                                </span>
                            </div>
                    ...
                    {showPermissionActions && (
                        <div className="mt-5 flex flex-wrap justify-center gap-3">
                            ...
                            >
                                <ShieldCheck size={16} />
                                <span>{t('nativeUpdate.gate.openSettingsAction')}</span>
                            </button>
                            <button
                                type="button"
                            ...
                            >
                                <Download size={16} />
                                <span>{t('nativeUpdate.gate.continueInstallAction')}</span>
                            </button>
                        </div>
                    )}
                    ...
                    {showRetry && (
                        <div className="mt-5">
                            ...
                            >
                                <RefreshCw size={16} />
                                <span>{t('nativeUpdate.gate.retryAction')}</span>
                            </button>
                        </div>
                    )}
                </section>
