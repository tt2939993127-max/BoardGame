...
当卡牌/角色仍需展示，但当前状态明确是“施工中 / 暂不可选 / 占位未完成”时，优先使用**中部横穿的斜向警示带**，而不是角落角标或整卡灰遮罩。

必须遵守：
- **只保留一根主斜带**：不要再叠第二块信息底、额外局部牌子或整卡黄纹遮罩。
- **斜带必须完整穿过卡面**：视觉上应从一侧切到另一侧，不能只剩底部半截或角落一小块。
- **文案直接放在斜带上**：默认使用 `施工中`、`开发中`、`暂不可用` 这类短语，禁止在带子上再挂次级解释块。
- **带子是状态覆盖层，不是布局元素**：必须 `absolute` 覆盖在卡面中部，不得挤压卡名、占位角标或其他 UI。
- **优先用于“保留展示但当前不可用”**：如果对象应该彻底隐藏，就不要再加斜带。

不应使用：
- 角落 pill 标签却要表达“整张卡当前不可用”
- 整卡压暗 + 多层黄黑纹理 + 中间再叠一块牌子
- 文字颜色与底色对比不足，导致 `施工中` 不可读
