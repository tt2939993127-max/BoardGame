...
#### Scenario: 启用中的游戏声明包交付信息
- **GIVEN** 某个启用中的游戏 manifest 需要在移动端通过独立包交付
- **WHEN** 运行时消费该注册表条目
- **THEN** 条目 MUST 显式暴露移动端包交付元数据
- **AND** 该元数据 MUST 能区分“首包内置”与“按游戏分包”
- **AND** 该元数据 MUST 能标识当前游戏对应的模块包与素材包标识
