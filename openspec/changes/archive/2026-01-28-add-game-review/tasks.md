...
- [x] 5.1 创建 `src/components/review/` 目录结构
- [x] 5.2 实现 `ApprovalBar.tsx` 好评率进度条组件
- [x] 5.3 实现 `ReviewItem.tsx` 单条评论组件
- [x] 5.4 实现 `ReviewList.tsx` 评论列表组件（含分页）

## 6. 评论表单

- [x] 6.1 实现 `ReviewForm.tsx` 评论表单组件
- [x] 6.2 实现好评/差评按钮组切换
- [x] 6.3 实现评论内容输入框（可选，限 500 字）
- [x] 6.4 实现提交逻辑（创建/更新）
- [x] 6.5 实现删除评论功能

## 7. 游戏评价区域

- [x] 7.1 实现 `GameReviewSection.tsx` 主组件
- [x] 7.2 实现游戏切换选择器（Tab 或下拉）
- [x] 7.3 集成 ApprovalBar、ReviewForm、ReviewList
- [x] 7.4 使用 React Query 管理数据获取与缓存

## 8. 主页集成

- [x] 8.1 在 `Home.tsx` 房间列表与排行榜之间添加 GameReviewSection
- [x] 8.2 调整布局确保视觉协调
- [x] 8.3 未登录状态下隐藏评论表单，显示登录提示
...
- [x] 10.1 测试未登录用户访问评论区
- [x] 10.2 测试重复提交评论（应更新而非创建）
- [x] 10.3 测试评价数 < 10 时的显示逻辑
- [x] 10.4 测试评论内容超长（500 字限制）
- [x] 10.5 测试关键词过滤触发
