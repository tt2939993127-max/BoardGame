...
#### Scenario: 未显式启用时保持普通执行
- **WHEN** 用户未明确开启检查/监察模式
- **THEN** 系统 MUST 保持普通执行模式
- **AND** 不得默认套用监察者模式的额外后置裁决、持续健康监控或更强通知策略

#### Scenario: 显式口令开启监察者模式
...
#### Scenario: 自动决策优先于定时巡检
- **WHEN** 首版产品范围需要在“定时 watcher/巡检”与“自动决策”之间取舍
- **THEN** 系统 MUST 优先实现自动决策
- **AND** 定时 watcher / cron / timer MAY 暂不启用或延后实现
- **AND** 不得把定时巡检作为首版自动决策可用性的前置条件

### Requirement: 简单决策 MUST 默认自动处理

系统 MUST 对单解、低风险、可逆的简单决策默认自动处理，而不是把它们上抛给用户。

#### Scenario: 单解低风险决策自动执行
- **WHEN** 某个决策属于单解、低风险且可逆的执行性选择
- **THEN** 系统 MUST 直接自动处理
- **AND** 不得为该决策打断用户

#### Scenario: 只有真实歧义或高风险项才上抛
- **WHEN** 决策涉及目标歧义、不可逆副作用、外部交付口径变化或高风险动作
- **THEN** 系统 MUST 将该项上抛给用户确认
- **AND** 在该确认中 SHOULD 顺带汇总此前已自动处理的低风险决策
