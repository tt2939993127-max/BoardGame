...
- [ ] 6.3 新增右侧 `属性` 面板，支持布局、外观、资源、动作分组编辑。
- [ ] 6.4 保持 authoring chrome 与正式内容布局解耦，不因开启编辑态改变页面真实排版。

## 7. Asset Authoring
- [x] 7.1 新增 `资源` 面板，展示当前 scene 的本地素材、R2 素材与待上传素材。
- [ ] 7.2 支持节点/皮肤从资源面板选择资源并回写 `assetRef`。
- [ ] 7.3 接入本地上传到 R2 的作者链路，并保持上传前后逻辑引用稳定。
...
- [ ] 9.4 运行至少 1 条 Home V2 作者态 E2E，验证选中、拖拽、保存、重新加载。
