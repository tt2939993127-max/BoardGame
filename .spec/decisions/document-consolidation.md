...
| 2026-08-12 | 中间态错误地把系统 `show-image-to-user` 与项目 `screenshot-delivery` 合成同名项目 skill | 系统 `D:\codex-home\skills\show-image-to-user\SKILL.md` + 项目 `.spec/skills/screenshot-delivery/SKILL.md` | 有语义调整，恢复正确分层 | 系统 skill 保持用户可见开图、多图交接、查看器和回退唯一真相；项目 skill 只保留 BoardGame evidence 路径、项目命令、标记脚本和相册边界；删除中间态 `.spec/skills/show-image-to-user/`，不丢失已迁移的项目专属信息。 |
