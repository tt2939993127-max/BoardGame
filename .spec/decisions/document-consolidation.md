...
| 2026-08-11 | 历史上 `.spec/skills/screenshot-delivery/SKILL.md`、`.spec/knowledge/standards/e2e-verification.md` 中重复的用户开图、多图编号、查看器和交付步骤 | 当时由系统 `D:\codex-home\skills\show-image-to-user\SKILL.md` 承担通用正文，项目层保留证据资格与 BoardGame 适配 | 历史裁决 | 这是当时的分层记录，不是现行入口。 |
| 2026-08-12 | 项目协作者需要只依赖仓库内容执行开图；旧 `screenshot-delivery` 已包含项目专属脚本与证据约定 | 项目 `.spec/skills/show-image-to-user/SKILL.md` | 有语义调整，完成项目迁入 | 将项目实际选用的系统开图规则与 BoardGame 专属证据目录、脚本、相册边界合并为项目唯一执行 skill；旧 `screenshot-delivery` 已删除。系统同名 skill 仅作以后人工比较的上游候选，不是本项目执行入口。 |
...
- P1 UI 组第一步曾以 `screenshot-delivery` 为 workflow；该历史入口的有效内容已迁入 `.spec/skills/show-image-to-user/SKILL.md`，旧文件已删除。`boardgame-ui-imagegen` 仍是 workflow，只引用 `e2e-verification.md`、`ui-change-gates.md`、`ui-ux.md`、`asset-pipeline.md` 等规范主源。
