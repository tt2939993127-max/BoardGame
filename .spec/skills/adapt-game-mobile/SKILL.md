...
- **图面审计是强制项**：跑完移动端 E2E 后，必须按项目 `.spec/skills/ui-audit-loop/SKILL.md` 对 `test-results/evidence-screenshots/_shared/` 中的原始截图逐张核图；没有逐张图面审计，就视为没有完成移动端验收。用户需要查看通过的图时，按项目 `.spec/skills/show-image-to-user/SKILL.md` 展示。
