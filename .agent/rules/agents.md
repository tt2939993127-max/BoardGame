...
> [!IMPORTANT]
> - **中文优先（强制）**：所有 UI 文本、注释、文档使用中文
> - **类型安全**：严格使用 TypeScript，避免 `any`
> - **组件复用**：优先提取通用组件到 `src/components/`
> - **强制验证**：**任何功能性变更（Feature/Bugfix）必须执行验证测试并提供证明（截图/日志）**
...
- 考虑代码复用和扩展性
- **UI 设计工作流**：
    1. **视觉打样**：使用 `generate_image` 生成 多种 种不同风格 
...
### 4. 验证测试 (强制)
> ⚠️ **所有功能变更完成后，必须执行此步骤，除非是仅外观修改等不涉及功能的变更**
...
- **自动化验证**：使用 `browser_subagent` 访问 `http://127.0.0.1:5174`
无法使用就用Chrome DevTools MCP 
- **交互测试**：模拟用户操作路径（如：点击按钮 -> 验证弹窗 -> 执行动作 -> 验证结果）
- **控制台检查**：读取浏览器控制台，确认无错误或警告
- **多人对局测试**：务必在 Debug Panel 中切换 Player ID (0/1/Spectator)，完整走通双人对局流程
...
**WebSocket 连接失败** → 修改 `vite.config.ts`：
```typescript
server: { 
  host: '0.0.0.0', 
  hmr: { host: '127.0.0.1' } // ❌ 不要设置 port，使用默认值
}
```

**错误 "Upgrade Required"** → 移除 `hmr: { port: ... }` 配置（避免与 server.port 冲突）
...
- **移动端兼容（强制）**：
    - 所有游戏必须适配移动端（手机/平板）
    - 使用动态视口单位 (`dvh`) 避免地址栏遮挡
    - 适配刘海屏/灵动岛 (`safe-area-inset`)
    - 触摸优化：最小点击区域 44px，避免粘性悬停
    - 响应式布局：优先使用 Flex/Grid 自适应，而非硬编码尺寸
