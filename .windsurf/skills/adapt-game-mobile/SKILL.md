...
1. 先看 `test-results/evidence-screenshots/` 里的首张主状态图。
2. 先用主状态图判断“整体比例、锚点、遮挡、假 hover”是否过关，再看局部交互截图。
3. 如果首张主状态图被任何浮层遮挡，先判这张图无效，再补拍新的无遮挡主状态图；不要带着无效截图继续宣布通过。
4. 汇报时必须同时写明：看了哪张图、从图里确认了什么、是否还有肉眼可见问题。
