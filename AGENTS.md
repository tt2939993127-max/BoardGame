...
> **涉及设计工具 / 视觉方案协作时，Open Design 是当前设计工具接入口：读 `docs/infra/open-design.md`，使用 `scripts/infra/setup-open-design.ps1` 或 `npm run setup:design:mcp` 接入；不得恢复旧设计工具 OAuth 或旧 MCP 接入。用户未指定交付形态而只说“设计稿 / 视觉稿 / 效果图”时，按 `docs/ai-rules/doc-index.md` 的设计批准门禁裁定默认交付物；如果用户明确点名 Open Design / OD / opendesign 作为设计交付或验收工具，Open Design artifact 及其导出审计截图可以作为设计稿候选，不得再强制切到 imagegen / 位图路线。无论哪条路线，都必须先读规则真相源与素材矩阵，AI 图面核验通过后才进入人工验收；不得用旧 artifact、失败 HTML、运行页截图或未审候选冒充设计稿。设计稿未获用户明确人工批准前，不得启动真实运行页、编写或扩展 Board/UI 实现、跑实现 E2E 或用运行页截图反推设计通过；固定构图类游戏 PC 桌面基线未通过前，不得转入移动端适配、移动端截图验收或移动端完成口径。**
...
- **症状形状必须和假设匹配**：若反馈显示的是“选了一部分后停住 / 只坏一部分 / 间歇发生”，禁止用“整段 suppress / 共享默认值污染 / 全量排序错误”这类会导致全程一致失败的假设直接收口，除非已有证据证明该假设确实能产生同样的部分症状。
...
- **写规范前必须先回答四个问题**：
  ...
  4. 该规范应该落在哪里：根 `AGENTS.md` / `docs/ai-rules/` / 项目 skill / 游戏专项 workflow / 用户故事，禁止把局部口径写成项目全局默认。
...
> **新增/改写 skill 先读分层说明（强制）**：先看 `./.codex/skill/README.md`，判断这次内容属于“系统通用能力”“项目 workflow”还是“项目 overlay”。**不要把系统 skill 正文整份复制到项目目录，也不要把项目事故细节塞回系统 skill。未经用户当轮明确要求，禁止把项目 skill 的新增条款、项目事故结论或项目 overlay 规则上升到 `D:\codex-home\skills\**`。**
...
> **通用规范定义（强制）**：根 `AGENTS.md` 与 `docs/ai-rules/` 默认是“所有游戏通用规范”，只能约束流程、方法、验证和文档要求，不能把某个游戏当前任务里的局部字段命名、卡牌结构或专用抓取源直接写成项目全局默认。游戏专属口径必须落到该游戏自己的 `rule/` 文档或专项规范里。
...
- **用户纠正使既有推断立即失效（强制）**：用户明确说“不是这个 / 不对 / 之前正确 / 只要某个对象 / 不要做某动作”或等价表达时，此前未被独立真相源证明的助手推断、相似案例、临时截图、当前代码、刚写的测试断言和由它们导出的方案必须立即降级为候选，不能继续当作事实。下一次会改变代码、配置、数据、测试或交付物的动作前，必须重新锁定：① 用户纠正后的目标与边界；② 能直接裁决的真相源；③ 要改的内容如何作用于该目标；④ 回到原始入口的验收口径。任一项未锁定时只能继续定位或说明缺口，禁止基于第二个猜测继续改、把猜测写进测试来证明自己，或借题扩展到用户未要求的重构、规范或旁支修复。用户指定“之前 / 原来 / 已正确”的历史基线时，当前实现、当前截图和新写断言不能替代历史证据；必须找到历史运行时、旧截图、旧代码、旧合同或用户明确确认的基线。
