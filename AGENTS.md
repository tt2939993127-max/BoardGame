...
3. **必须自审截图（强制）**：
   ...
   - 使用 `mcp_image_viewer_view_image` 查看 `test-results/` 目录中的所有测试截图
...
- **E2E 测试必须通过并自审截图（强制）**：
  ...
  - **必须用 MCP 查看测试截图（强制）**：
    ...
    - **禁止假设截图内容**：必须实际查看图片，不能凭想象或猜测
  - **用户上传的截图直接读取（强制）**：
    - 用户通过聊天上传的截图（拖拽到聊天框）会自动显示在对话中
    - **禁止使用 MCP 工具读取用户上传的截图**，直接从对话中查看即可
...
- **E2E 测试必须由 AI 自主运行（强制）**：
  ...
  - **证据文档必须包含截图（强制）**：
    ...
    * **没有截图禁止写证据文档**：如果 `test-results/` 目录为空或没有相关截图，必须先修复测试让它通过并生成截图，禁止写空的证据文档
    ...
    * **更新证据文档后必须打开截图文件夹**：使用 `explorer test-results` 打开截图所在文件夹，方便用户查看，禁止打开 `evidence/` 文件夹
  - **证据文档格式**：
    ```markdown
    # <功能名称> E2E 测试 - 运行结果
