...
3. **必须自审截图**：优先查看 `test-results/evidence-screenshots/` 下的显式证据截图；失败用例再补看 `test-results/playwright-artifacts/` 下的自动产物，确认 UI 正确、交互完整，并且**已经满足本轮需求/用户反馈的目标**
4. **必须实际打开截图，不得只看测试通过**：所谓“自审截图”必须真的打开图片逐张检查，不能只根据测试通过、文件存在或代码逻辑推断 UI 正确；**验收口径不是“图里有界面”或“元素出现了”，而是“图里结果已符合本轮需求”**
5. **证据文档必须写出肉眼观察结论**：每张关键截图至少写出 1-3 条人工观察到的具体现象，例如“左侧第一个基地不再被切边”“泰坦仍在基地上方中间”“持续行动之间没有额外缝隙”；**结论必须直接对应本轮需求点/验收点**，禁止只写“截图正常”“UI 正确”“看起来没问题”
6. **必须先看图再写结论**：结论、证据文档、对用户的汇报都必须建立在“已经实际打开并看过图，且已对照需求确认符合目标”的前提上；禁止先写“已修复/正常”，再回头补看图
7. **外部 R2/CDN 资源缺失时仍必须看图**：如果测试环境没有稳定拉到牌面美术，必须在结论里明确说明“资源未渲染”，并把验收重点放在布局、相对大小、层级、遮挡、溢出、对齐；禁止把“白卡面”直接当成布局失败，也禁止借此跳过布局检查
11. **必须逐条核对用户本轮指出的图片问题**：如果用户这轮明确指出了“底部多出色块”“日志挡住悬浮球”“没有必要的位移/收口”“元素仍溢出/遮挡”等具象视觉问题，验收时必须逐条对照这些问题检查截图，并在结论里明确写出“该问题是否还存在、具体出现在图的哪里”；**禁止只写总体正常而不回应用户指出的图像问题**
12. **图片里出现新异常时不得宣称收口**：即使原测试断言通过，只要截图里出现新的明显视觉异常、遮挡、空白块、层级错误、异常位移或与用户目标冲突的现象，就必须先把该异常当成 blocker 处理或明确上报；**不得忽略图片内容继续对用户说“已修复/已收口”**
...
- **App/网页边界隔离（强制）**：Android/iOS 壳层能力（如 OTA、原生 APK 更新、返回桥接、原生插件 UI、包管理入口）与网页端必须做显式边界隔离。**禁止**只用 `import.meta.env.MODE === 'android'`、构建产物名、渠道名之类“构建时信号”直接决定运行时行为；这些信号最多只可用于裁剪包体、隐藏调试路由、切换构建配置。凡是会影响用户可见 UI、自动副作用、原生插件调用、更新检查、原生返回行为的逻辑，必须再经过“真实原生运行时”门禁（如 `Capacitor.isNativePlatform()`、`Capacitor.getPlatform()`、原生桥存在性检测或统一运行时探测函数）。共享组件若同时服务网页与壳层，必须优先使用统一探测函数，禁止把壳层专属行为直接泄漏到网页端。
- **App/网页边界统一探测（强制）**：项目内凡是需要判断“当前是否为原生 Android/iOS 壳运行时”的地方，必须复用同一个统一探测函数/模块；**禁止**在页面、组件、hook、service 内各自直接写 `window.Capacitor`、`window.androidBridge`、`Capacitor.isNativePlatform()`、`getPlatform()` 的散点判断。若某处需要更严格或更宽松的门禁，必须先回收进统一探测层，再由调用方使用带明确语义的封装函数，不能各写一套。
- **App 专属启动副作用门禁（强制）**：凡是 `main.tsx`、`App.tsx`、Provider、Manager、bootstrap 入口里的原生壳专属启动逻辑（如 `notifyAppReady`、热更监听注册、原生更新检查、返回桥接、原生包状态 hydration、原生插件预热），必须在**进入函数前**先通过统一运行时门禁；**禁止**把 App 专属逻辑作为网页默认启动路径的一部分，再在函数内部“自行早退”。
- **网页隔离回归断言（强制）**：只要新增或修改了 App 专属能力，同时存在网页入口，就必须至少验证一条“网页端不会出现该能力 UI/副作用”的回归口径。最低要求包括：网页首页/大厅不能出现 App 检查更新/热更文案、网页启动不能触发 App 热更或原生更新检查、网页路由切换不能挂载返回桥或原生插件 UI。若没有自动化测试，至少在证据文档中明确写出人工核对结果与截图路径。
...
- **仓库根目录分支职责规范（强制）**：当前仓库根目录默认只承担 `main` 主工作树职责，**本目录禁止切换到其他分支**。若任务需要进入任意非 `main` 分支，必须新开对应 `git worktree`，并在该 worktree 中开展、验证和提交该分支的工作；禁止把仓库根目录当作通用工作区在不同分支之间来回切换。
...
- **命令超时策略（强制）**：从现在起，所有 shell/脚本/测试/推送命令默认使用 **最长 30 分钟** 超时（`1800000ms`）。这表示超时上限，不是要求命令必须运行满 30 分钟；命令提前完成就立刻结束。不要再先用 120 秒、180 秒之类的短超时试跑。
- **Push 超时策略（强制）**：用户明确要求执行 `push` 时，默认直接给 **最长 30 分钟** 超时，不要先用更短超时试一次再重跑；若网络层出现 `408`、`SSL_ERROR_SYSCALL`、sideband 断流等上传抖动，优先在说明原因后延长超时或切换 `HTTP/1.1` 后重试，而不是先把任务判成失败。
- **文件移动/复制规范（强制）**：
...
- **E2E 测试必须通过并自审截图（强制）**：
  ...
  - 必须实际打开截图逐张确认，不得只根据“测试通过”或“代码看起来没问题”就宣称已看图；**也不得只因截图里“有界面/有按钮”就视为验收通过，必须对照本轮需求确认图中结果已经满足目标**
  - 必须先完成看图和需求对照，再写证据文档和对用户的结论；禁止“先下结论后补看图”
  - 证据文档中的截图分析必须写出肉眼可见的具体现象，至少包含布局位置、遮挡/溢出、间距/对齐三类中的一类；**并明确这些现象为什么足以证明符合本轮需求**，禁止只写“正常”“符合预期”
...
- **素材数据录入规范（强制）**：根据图片素材提取业务数据时，必须全口径核对、逻辑序列化、关键限定词显式核对，输出 Markdown 表格作为核对契约。具体流程、来源优先级、图片索引处理和“先文档后实现”要求，统一以 `docs/ai-rules/data-entry.md` 为准。**图片文字辨识零猜测原则（强制）**：任何文字（名称、描述、数值、关键词）只要有一点看不清或不确定，必须立即停止当前数据录入工作，向用户说明哪些位置无法辨认，并索要更清晰的图片。禁止根据上下文、常识或英文原版"猜测"看不清的中文文字。已猜测录入的数据视为缺陷，必须重新核对。**录入完成后的资源上传默认由 AI 主动执行（强制）**：只要本轮录入新增、替换或移动了运行时会访问的图片/音频/atlas/裁图资源，就必须在收尾前主动完成 manifest 重建、R2/CDN 上传与远端可访问性回查；**没有上传不算完整交付**。若因当前环境、配置、脚本失败或用户明确要求而暂时不能上传，必须在最终汇报中明确写出“哪些资源尚未上传、为什么没上传、当前运行态会受什么影响”，禁止不提。
- **图集语义零猜测（强制）**：涉及 `atlas`、`crop`、`previewRef`、`hand atlas`、卡图切片映射或类似资源接线时，**禁止**根据素材外观直接推断运行时语义。空白格、黑边、角落装饰、额外立绘、看起来像“两张塞一格”之类的视觉现象，都**不能**直接推出“这不是卡”“需要 split/topCrop”“一格对应两张”或“必须改 atlas rows/cols”。处理顺序只有两种：1）先对照同游戏旧实现、现有运行时接线和专项规范；2）若仍无法唯一确定，先问用户裁决。未经确认不得自行改图集使用方式。
