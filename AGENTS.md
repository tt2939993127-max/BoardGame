...
- `docs/architecture/ui-dual-platform-architecture.md` — **涉及双端 UI 架构、页面壳层/游戏壳层分层、共享组件如何同时服务网页与 App、以及“该缩放还是该重排”这类架构决策时必读**。含推荐分层、职责边界、终端适配真值来源、布局 preset 选型与迁移顺序。
...
4.1. **必须精确检查与业务问题直接相关的图像区域**：看图时不能只做“整页扫一眼”或只确认页面能打开，必须逐项核对用户这轮报的问题位点本身，例如具体那张牌、具体那个候选列表、具体那个基地槽位、具体那段日志遮挡区、具体那个弹窗步骤；**未检查业务问题对应区域，不算完成看图**
4.2. **展开类问题必须把“触发控件 + 展开内容完整边界 + 关键参照物”同时拍进主截图**：凡是 `undo` 展开、`action-log` 展开、下拉菜单、弹出面板、抽屉、tooltip、popover、弹窗步骤错位/遮挡/下坠/溢出类问题，主截图必须同时看见：1）被点击的那个控件本体；2）展开出来的完整内容边界；3）用于判断相对位置的兄弟按钮、被遮挡区域或屏幕边界。**只拍到按钮高亮、只拍到面板一角、只拍到展开内容的一部分、或截图已经把关键边界裁掉，一律不得判通过**
4.3. **截图里看不全展开部分，默认视为“未证明修复”**：如果主截图没有把展开内容完整收进画面，导致无法肉眼判断“它到底贴着谁、有没有掉到别的按钮层级、有没有超出屏幕、有没有挡住目标区域”，则不论断言是否通过，都只能算“未证明”，不得对用户汇报为已修复
...
8. **截图必须直击问题场景，禁止拿相关界面代替问题界面**：如果用户报告的是“游戏内手牌预览不对 / 对局里图标不显示 / 结算时遮挡 / 某个弹窗在实战流程里异常”，验收截图就必须来自对应的**游戏内实际场景**；禁止拿大厅页、资源诊断页、独立预览页、只展示局部组件的截图替代“问题本身是否修复”的证据
9. **截图必须证明“问题已消失”，不是只证明“相关功能存在”**：例如用户说“游戏内手牌图不对”，则截图必须能看到实际对局中的那张手牌、其周边上下文，以及原问题位点现在的样子；仅证明“手牌区有牌”“资源 URL 正确”“某张牌在别的页面能显示”都不算验收通过
9.1. **共享组件问题不得用兄弟入口截图冒充主证据**：如果用户报的是共享 HUD/FAB/Modal 里的某个具体入口，例如“撤回展开位置错了”，主截图就必须是该入口本身展开后的图；拿同一套组件里的 `settings`、`action-log`、`feedback`、或另一个游戏里更容易出图的入口，只能作辅证，不能作“问题已解决”的主证据
10. **游戏内问题优先核对完整链路截图**：只要问题发生在对局流程中，优先保留从真实入口进入后的游戏内截图，至少覆盖“进入问题场景后的主状态图”；除非用户明确接受或该问题本来就是独立预览页问题，否则不得用脱离对局上下文的截图充当主证据
...
12.1. **端到端必须覆盖业务真实问题链路，禁止只造一个会出图的代理场景**：如果用户报的是“某阶段进入下一阶段后列表没刷新”“某张牌结算后仍可再次被选”“某个实战流程里 UI 仍显示旧对象”，E2E 就必须覆盖该真实业务链路、真实触发时机和真实问题位点；**只证明通用机制在一个自造示例里工作、或只证明页面能渲染出一张图，都不能算业务问题已通过**
13. **必须创建证据文档**：`evidence/<功能名>-e2e-test.md`，嵌入截图并分析内容
14. **绝对禁止用单元测试糊弄**：UI 交互/多玩家协作/动画特效 → 必须用 E2E 测试
15. **测试失败处理**：加日志定位根因 → 审查调用链 → 修复代码或调整测试
...
- **“正常操作”反馈不得偷换为误操作（强制）**：当用户明确说明“我是正常操作”“按正常流程也会触发”“不是误点/乱点”时，必须先把这句话当作本轮排查前提，沿着**正常用户路径**做全链路分析。**禁止**未经证据就先把问题解释成用户点错、阶段理解错、操作越界，或擅自把问题重写成“某个边界阶段不该点”。若最终判断用户描述与代码事实不符，必须拿出具体链路证据再说明，而不是先入为主替用户改写语义。
- **用户报 bug 时不得先怀疑“你看错了”（强制）**：当用户明确反馈“这里会闪一下 / 刷新一下 / 不对 / 有问题 / 就是 bug”时，默认将该反馈视为**真实体验问题**进入排查。**禁止**先用“可能只是动画”“也许是正常表现”“会不会你看错了/记错了/感觉错了”这类表述给问题降级。即使最终根因是文案、动画、视角切换、过渡表现或认知落差，只要用户明确感知到异常，也应先按 bug 处理，再用证据细分根因；若要否定用户观察，必须先拿出可复查的链路证据、截图或日志。
...
- **页面语义边界隔离（强制）**：首页/大厅、游戏页、App 壳层专属页、后台、开发工具页等**文案命名空间、加载态、空状态、错误态、fallback、骨架屏、悬浮提示**必须按场景隔离。**禁止**把 `matchRoom.*`、`home.*`、`admin.*`、`devTools.*`、`ota.*` 等场景语义直接跨页面复用成默认文案；如果只是复用视觉容器，文案语义也必须由调用方按当前页面显式传入。
- **共享 Loading/Fallback 组件默认值约束（强制）**：任何会被多个页面/路由复用的 `LoadingScreen`、`RouteFallback`、`Suspense fallback`、空状态、错误兜底组件，**禁止内置带业务场景含义的默认文案**。允许的默认值只能是中性语义（如“正在加载页面...”），或直接要求调用方必传 `title/description/progressText/namespace`。若组件默认值里出现具体页面语义（如“加载对局资源”“进入房间中”“安装新版 App”），则该组件只能在该页面族内使用，不得升格为通用组件。
- **UGC 待移除默认忽略（强制）**：`UGC`、Builder、RuntimeView、Sandbox 及其相关入口默认视为**待移除模块**。除非用户在本轮任务中明确点名要求处理，否则**不得**因为“全链路完整性”“顺手保持兼容”“统一入口”“历史模块一起改”而把 `UGC` 纳入方案设计、页面边界、自测清单、回归口径、文案命名空间、路由 fallback、共享组件默认值或架构取舍。需要提及目录结构时，可保留事实描述，但不得再把 `UGC` 当作当前平台的默认活跃能力。
...
- **跨页面改动反向影响自检（强制）**：凡是修改了 `App.tsx`、路由级 `Suspense fallback`、全局 Modal/Toast/Loading 组件、全局 Provider、运行时探测、共享 hook、共享文案 key 或任意“多个页面都会碰到”的入口逻辑，交付前必须反向检查“未修改页面是否被带偏”。最低要回答四件事：1）首页不会出现游戏页语义；2）游戏页不会出现首页/大厅语义；3）网页端不会出现 App 壳语义；4）后台/工具页不会继承业务主流程文案。**禁止**只验证目标页面正常，就默认判定共享改动安全。
...
- **--no-verify 使用规范（强制）**：
  ...
  - **默认禁止无校验推进**：除非用户明确要求“直接上传”“先 push 再说”“无校验上传”“跳过验证”或等价口径，否则不得为了赶进度、规避门禁或图省事直接使用 `--no-verify` 提交 / push。
...
- **默认校验门禁（强制）**：只要本次任务不是用户明确批准的“无校验推进”，就必须在提交、push、对外宣称完成前完成与改动相匹配的最小必要校验；不能把“先推上去再看”“用户暂时没提测试”“只是顺手小改”当作跳过验证的理由。
...
- **回归性问题先定位最后正常版本（强制）**：必须先用 `git log`、`git show`、现有 evidence 文档、历史截图或测试产物定位“最后正常”的代码版本或行为证据；在最后正常版本未定位前，不得把当前现象直接当成一个全新问题自由发挥。
- **回归修复默认优先恢复历史正确行为（强制）**：如果已经找到最后正常版本，且历史实现仍兼容当前架构，默认优先手工恢复那一版的正确行为；禁止先自创一个“看起来也行”的新布局、新交互或新算法来替代历史恢复。
- **未完成历史比对前禁止先造方案（强制）**：只有在明确确认“历史实现已不存在、无法直接恢复、或与当前结构冲突”后，才允许提出新方案；并且必须在回复里写清为什么不能直接恢复、最后正常证据来自哪里、当前方案相对历史行为的差异是什么。
- **回归证据来源必须写清（强制）**：如果最后正常版本不是直接来自当前工作树代码，而是来自历史截图、evidence 文档、线上产物或用户提供截图，必须明确标注证据文件/截图路径，禁止只写“之前有一版是好的”这类不可复查表述。
