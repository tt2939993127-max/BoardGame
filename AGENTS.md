...
- **主界面 UI 准入（强制）**：面向用户的主入口、总控页、首页、工作台首页中，每个 UI 元素都必须具备直接用户价值，至少满足以下之一：1）帮助用户做决策；2）帮助用户执行当前任务；3）帮助用户理解当前状态且无法由更轻量文案替代。仅服务开发、调试、内部编排、实现细节暴露的元素（如内部模块编排链、调试开关、编辑器控件、无动作价值的流程示意）默认禁止进入主界面；若确需保留，必须能明确回答“用户缺少它会损失什么”，否则直接删除或下沉到调试态。
- **中文优先（强制）**：所有交互、UI 文本、代码注释、设计文档必须使用中文。
- **后台/工作台 UI 默认中文（强制）**：Flowise 后台、AI Repo Workbench、管理页、列表页、弹窗、按钮、空状态、提示语等面向用户的 UI，默认必须直接落中文文案；禁止用英文菜单名、英文页标题、英文按钮或英文空状态充当占位，除非该术语没有公认中文且用户明确要求保留英文。
