...
- **口语化提交口令单独解释**：像“看下没问题就提交”“检查没问题就推”这类表达，仍按“先做静态分析/门禁自检，再提交/推送”理解；只有明确说出“无校验 / 直接提交 / 直接 push / 不跑测试”时，才进入快速路径。
...
> **提交前必须先跑 `npm run i18n:check`**：只要本轮准备提交，且用户没有明确进入无校验快速路径，就必须先执行一次 `npm run i18n:check`；未跑不得提交，发现缺 key 必须先补齐，禁止在 pre-push 才临时找补。
