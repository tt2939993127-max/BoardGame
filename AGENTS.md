...
- **Git 变更回退与暂存规范（强制）**：所有 Git 回滚命令必须先说明原因并获得用户明确许可。**修复 bug 时必须全部手动修改代码（strReplace/editCode），无视 token 和时间消耗，禁止用 git restore 恢复文件后再修改**。
...
- **--no-verify 使用规范（强制）**：
  ...
  - **正确做法**：修复所有 errors 后再提交，无法立即修复则使用 `git stash`
- **文件移动/复制规范（强制）**：
...
- **strReplace 工具禁用场景（强制）**：
  - ❌ **绝对禁止**：对包含大量中文注释/字符串的大文件（>1000 行）使用 `strReplace` 工具
  - ❌ **绝对禁止**：对 `src/games/*/domain/index.ts` 等核心领域文件使用 `strReplace` 工具
  - ⚠️ **原因**：`strReplace` 工具会破坏整个文件的 UTF-8 编码，导致所有中文变成乱码（如 `检测到` → `妫€娴嬪埌`）
  - ✅ **正确做法**：直接使用 Node.js 脚本（`.mjs`）并显式指定 `'utf-8'` 编码
- **修改前强制验证（强制）**：
  ...
  - ✅ **强制流程**：
    ...
    3. 确认问题后再修改，不得跳过验证步骤
- **包含中文文件的修改流程（强制）**：
  - ✅ **唯一正确做法**：使用 Node.js 脚本（`.mjs`）
  - ❌ **绝对禁止**：先用 `strReplace` 尝试，失败后再用脚本（浪费时间且可能破坏文件）
  - ✅ **判断标准**：文件包含 >50 行中文注释/字符串 → 直接用 Node.js 脚本
  - ✅ **脚本模板**：
    ```javascript
    import { readFileSync, writeFileSync } from 'fs';
    const content = readFileSync('file.ts', 'utf-8'); // 显式指定 utf-8
...
- **文件操作工具选型决策树（强制）**：
  ...
  禁止降级到 PowerShell 写入（仅允许只读）
  ```
  - **Node.js 脚本**：大段代码（>50 行）、精确行号定位、复杂逻辑、多文件操作、**包含大量中文的文件**
  - **IDE 工具（strReplace/editCode）**：单文件小范围修改（<50 行）、简单替换、**且文件中文内容少**
