...
- SmashUp：派系选择页移动端紧凑布局、顶部回合提示贴纸点击不应误开详情

## 结论
- **DiceThrone 达标**：整页选角界面恢复为页面内 overlay 后，左侧角色列表、加入中、连接中、进入游戏 HUD 均未再出现整体右偏。
- **SmashUp 达标**：移动端横屏已恢复 2 列紧凑布局；顶部回合提示贴纸点击不会误触发派系详情。
...
- 我实际看到：
  - 横屏下派系列表为 2 列，每列卡面之间有明确间距。
  - 第三张卡已经换到下一行，不再是一排 3 张硬挤。
  - 标题区与顶部提示贴纸居中，整体没有明显右偏。
- 验收判断：**达到“移动横屏不再过挤/不再偏移”的标准**。
