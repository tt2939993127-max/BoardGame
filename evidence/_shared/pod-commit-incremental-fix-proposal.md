...
1. 导出所有被修改的文件
   ```bash
   git show --name-status 6ea1f9f | grep -E "^[AMD]" > evidence/_shared/pod-commit-all-files.txt
   ```

2. 按模块分组统计
   ```bash
   # DiceThrone
   grep "src/games/dicethrone/" evidence/_shared/pod-commit-all-files.txt | wc -l
   
   # SummonerWars
   grep "src/games/summonerwars/" evidence/_shared/pod-commit-all-files.txt | wc -l
   
   # 引擎层
   grep "src/engine/" evidence/_shared/pod-commit-all-files.txt | wc -l
   
   # 框架层
   grep "src/components/game/framework/" evidence/_shared/pod-commit-all-files.txt | wc -l
   
   # 服务端
   grep "src/server/" evidence/_shared/pod-commit-all-files.txt | wc -l
   ```
...
**输出**: `evidence/_shared/audit-report-summonerwars.md`
