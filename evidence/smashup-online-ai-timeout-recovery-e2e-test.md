...
- 基地上的 `影舞者` 仍保留在场上，说明这次处理不是执行原本的 sacrifice 选择，而是取消卡住交互后直接收掉 AI 当前回合。
...
- 处理后：
  - 补丁状态：`rejectedCount > 0`、`delegatedCount === 1`、`forceEndTurnDelegated === true`
  - 权威状态：`sys.interaction.current === null`
