...
- 本文档中“`cthulhu_corruption` 应收敛到 `targetType: 'generic'`”的结论已失效。
- 失效原因不是“历史判断完全无依据”，而是 2026-04-26 已重新做真实页面 E2E，证明当前前端链路可以稳定完成：
  - 手牌双击真实打出 `腐化`
  - 棋盘直点随从
  - 目标随从被消灭
  - 手牌新增 `special_madness`
  - 流程回到可继续出牌状态
- 新的收口证据见：
  - `evidence/smashup/smashup-cthulhu-corruption-direct-select-e2e-2026-04-26.md`
- 因此，这份 2026-04-23 文档只能作为“当时线上问题与临时 generic 收敛方案”的历史记录，不能继续充当当前实现的正确性依据。
