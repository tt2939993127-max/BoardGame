...
## 2026-04-26 第七轮回写：Skeletons 剩余问题继续收敛
...
- 本轮新增 / 回写测试：
  - `skeletons_lord_of_bones 天赋可挖掘这里任意埋葬牌而不只限自己`
  - `skeletons_grave_goods 首次埋葬后若只剩一张手牌不能额外埋葬另一张`
  - `skeletons_grave_goods 额外埋葬时可选择不同基地`（现已显式断言**弃掉的是 discard-cost，额外埋葬的是 bury-extra**）
- 本轮验证：
...
| 28 | Diva | 女主角 | 2026-04-29 已补“可选复制标准行动且最终只得 +2”的真实入口 E2E 证据 |
...
| 32 | Aramis | 阿拉密斯 | 2026-04-29 已补“自己被标准行动影响后提供额外行动”的真实入口 E2E 证据 |
...
| 35 | Smart Set-Up | 聪明Set-Up | 2026-04-27 已补“附着到其他玩家随从后，在该基地首次打出随从时抽 1 张”的真实入口 E2E 证据 |
| 36 | Shark Tattoo | 鲨鱼纹身 | 2026-04-26 当前静态数据与 locale 已一致 |
| 37 | It's Blitzin' Time! | 现在是闪电时间！ | 2026-04-27 已补“打出后选择己方随从，本回合 +3”的真实入口 E2E 证据 |
| 38 | Kaiju Conflict | 怪兽冲击 | 2026-04-26 已按卡图优先重录中文名 |
| 39 | Eh? | 嗯？ | 2026-04-26 当前静态数据与 locale 已一致 |
| 40 | Fast as Lightning | 快如闪电 | 2026-04-29 已补“打到阿拉密斯后进入《女主角》《阿拉密斯》联合反应窗”的真实入口 E2E 证据 |
| 41 | Bewitched | 着魔 | 2026-04-26 已按卡图优先重录中文名 |
| 42 | Mouse, Bird and Sausage | 老鼠、鸟和香肠 | 2026-04-26 已按卡图优先重录中文名 |
| 43 | High-Speed Chase | 高速追逐 | 2026-04-27 已补“天赋转移行动、移动己方随从并给予 +3”的真实入口 E2E 证据 |
...
- 2026-04-29 再补记（三）：
  - 《快如闪电》《女主角》《阿拉密斯》已补浏览器级真实入口证据，见 `evidence/smashup/smashup-world-champs-diva-aramis-fast-as-lightning-e2e-2026-04-28.md`。
  - 《女主角》旧“实现已正确”结论失效：此前只看 `events` 没看 `finalState`，漏掉了 `smashup_reaction_choose` 双 reduce 导致的 `+4`。
  - 《阿拉密斯》本轮确认是 `onMinionAffected` 触发范围错误，不是数据录入或索引错误。
  - 截至本轮，`World Champs` 已累计补到 `19` 条正路径对象级 L3 证据；但三新派系整包仍不能据此直接改判为“已收口”。
