...
- `世界冠军` 的 `previewRef.index` 整组录错，导致：
  ...
  - UI 显示的是另一张卡的整张卡面；
  - 因此会出现“看起来像打出武士酱，却触发海龟阿凯效果”这类现象。
...
- 所以用户看到“武士酱”卡面时，底层实际可能是 `海龟阿凯`，打出后就会弹出 `海龟阿凯` 的交给对手手牌并抽两张交互。

- `斯通福德` 卡面实际在 `index 31`。
- 修复前代码把 `index 31` 绑定给了 `world_champs_mummy`。
- 所以用户看到“斯通福德”卡面时，底层实际可能是 `木乃伊`，自然不会触发 `斯通福德` 的打出检索行动卡效果。
...
- 根因明确是 `世界冠军 cards7 图集索引错位`，不是 `武士酱 / 海龟阿凯 / 斯通福德` 的能力实现写错。
